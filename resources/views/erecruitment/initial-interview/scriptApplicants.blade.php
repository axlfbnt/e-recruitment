<script>
    $(document).ready(function() {
        ...
        $('.button-pass').on('click', function() {
            handleBulkAction(vacancyId, 'Follow-up Interview');
        });
        $('.button-reject').on('click', function() {
            handleBulkAction(vacancyId, 'Failed Interview Test');
        });
        ...
        $('.button-invite').on('click', function(event) {
            ...
            if (form.checkValidity() === false) {
                ...
                if (selectedApplicants.length > 0) {
                    ...
                    $.ajax({
                        url: "{{ url('/initial-interview/approvalAdministrative') }}" + "/" +
                            vacancyId,
                        type: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
        ...
        function handleBulkAction(vacancyId, status, applicantIds = null) {
            ...
            if (selectedApplicants.length > 0) {
                $.ajax({
                    url: "{{ url('/initial-interview/approvalAdministrative') }}" + "/" + vacancyId,
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status,
        ...
        $('body').on('click', '[id^=passMenu]', function() {
            ...
            handleBulkAction(vacancyId, 'Follow-up Interview', [applicantId]);
        });
    ...
    $('#importModal form').on('submit', function(event) {
        ...
        $.ajax({
            url: "{{ route('initial-interview.import') }}", // Rute untuk mengimpor file
            type: 'POST',
            data: formData,
