<script>
    $(document).ready(function() {
        ...
        $('.button-pass').on('click', function() {
            handleBulkAction(vacancyId, 'Final Interview');
        });
        ...
        $('.button-invite').on('click', function(event) {
            ...
            if (form.checkValidity() === false) {
                ...
                if (selectedApplicants.length > 0) {
                    ...
                    $.ajax({
                        url: "{{ url('/followup-interview/approvalAdministrative') }}" + "/" +
                            vacancyId,
                        type: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
        ...
        function handleBulkAction(vacancyId, status, applicantIds = null) {
            ...
            if (selectedApplicants.length > 0) {
                $.ajax({
                    url: "{{ url('/followup-interview/approvalAdministrative') }}" + "/" + vacancyId,
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status,
        ...
        $('body').on('click', '[id^=passMenu]', function() {
            ...
            handleBulkAction(vacancyId, 'Final Interview', [applicantId]);
        });
    ...
    $('#importModal form').on('submit', function(event) {
        ...
        $.ajax({
            url: "{{ route('followup-interview.import') }}", // Rute untuk mengimpor file
            type: 'POST',
            data: formData,
