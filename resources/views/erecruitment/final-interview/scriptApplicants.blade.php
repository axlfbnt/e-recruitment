<script>
    $(document).ready(function() {
        ...
        $('.button-pass').on('click', function() {
            handleBulkAction(vacancyId, 'Offering');
        });
        ...
        $('.button-invite').on('click', function(event) {
            ...
            if (form.checkValidity() === false) {
                ...
                if (selectedApplicants.length > 0) {
                    ...
                    $.ajax({
                        url: "{{ url('/final-interview/approvalAdministrative') }}" + "/" +
                            vacancyId,
                        type: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
        ...
        function handleBulkAction(vacancyId, status, applicantIds = null) {
            ...
            if (selectedApplicants.length > 0) {
                $.ajax({
                    url: "{{ url('/final-interview/approvalAdministrative') }}" + "/" + vacancyId,
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status,
    ...
    $('#importModal form').on('submit', function(event) {
        ...
        $.ajax({
            url: "{{ route('final-interview.import') }}", // Rute untuk mengimpor file
            type: 'POST',
            data: formData,
