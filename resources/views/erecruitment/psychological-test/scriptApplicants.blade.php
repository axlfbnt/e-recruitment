<script>
    $(document).ready(function() {
        ...
        $('.button-pass').on('click', function() {
            handleBulkAction(vacancyId, 'Initial Interview');
        });
        $('.button-reject').on('click', function() {
            handleBulkAction(vacancyId, 'Failed Psychological Test');
        });
        ...
        $('.button-invite').on('click', function(event) {
            ...
            if (form.checkValidity() === false) {
                ...
                if (selectedApplicants.length > 0) {
                    ...
                    $.ajax({
                        url: "{{ url('/psychological-test/approvalAdministrative') }}" + "/" +
                            vacancyId,
                        type: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
        ...
        function handleBulkAction(vacancyId, status, applicantIds = null) {
            ...
            if (selectedApplicants.length > 0) {
                $.ajax({
                    url: "{{ url('/psychological-test/approvalAdministrative') }}" + "/" + vacancyId,
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status,
        ...
        $('body').on('click', '[id^=passMenu]', function() {
            ...
            handleBulkAction(vacancyId, 'Initial Interview', [applicantId]);
        });
        ...
        $('body').on('click', '[id^=failMenu]', function() {
            ...
            handleBulkAction(vacancyId, 'Failed Psychological Test', [applicantId]);
        });
    });
